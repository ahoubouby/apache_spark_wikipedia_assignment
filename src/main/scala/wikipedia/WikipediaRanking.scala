...
object WikipediaRanking extends WikipediaRankingInterface {
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.filter(el => el.mentionsLanguage(lang)).count().toInt
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(wiki => langs.map(l => if (wiki.mentionsLanguage(l)) {
            (l, wiki)
          } else {
            null
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = index
    .mapValues(article => article.count(_ => true))
    .filter(_._2 != 0 )
    .collect()
    .toList
    .sortBy(_._2)(Ordering.Int.reverse)
  ...
  val result =   for {
    wiki <- rdd
    lang <- langs
  } yield  ((lang, if (wiki.mentionsLanguage(lang) ) 1 else 0 ) )
    result
      .filter (_._2  != 0 )
      .reduceByKey(add)
      .collect()
...
//    rdd
//    .flatMap(wiki => langs.map( l => (l, wiki.mentionsLanguage(l) match {case true => 1 case false => 0})))
//    .reduceByKey(add)
//     .collect()
//     .toList
//     .filter(_._2 !=  0 )
